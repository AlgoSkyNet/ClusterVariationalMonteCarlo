...
#ifndef MCMEASUREMENTSTAGGEREDMAGNETIZATION1D_H
#define	MCMEASUREMENTSTAGGEREDMAGNETIZATION1D_H

class mcMeasurementStaggeredMagnetization1D : public mcAbstractMeasurement {
public:
    mcMeasurementStaggeredMagnetization1D(){}
    mcMeasurementStaggeredMagnetization1D(mcConfiguration* conf, mcAbstractVector* twf) {
    ...
    void measurement(mc_state s) {
        ...
        for (int i = 0; i < theConfiguration->nTotalSites; i++) {
            tbit = -(((upstate & (ONE << i)) >> i)-((downstate & (ONE << i)) >> i))*(1-((i&1)<<1));
//            ret += tbit.get_d();
